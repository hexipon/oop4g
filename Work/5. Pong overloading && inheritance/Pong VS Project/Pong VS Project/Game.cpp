...
_Game::_Game(sf::RenderWindow& _window, float& _deltaTime) :
	State(_window, _deltaTime)
{
	//setup walls
	Walls[Wall::TOP].init(ball);
	...
	Score::player1Score().SetPos(Vec2f((GameData::SCREEN_RES.x / 2) - (GameData::SCREEN_RES.y*GameData::SCORE_FONT_SIZE)*1.5, GameData::SCREEN_RES.y*.1f));
	...
	Score::player2Score().SetPos(Vec2f((GameData::SCREEN_RES.x / 2) + (GameData::SCREEN_RES.y*GameData::SCORE_FONT_SIZE)*.5f, GameData::SCREEN_RES.y*.1f));
