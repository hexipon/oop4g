...
void gameManager::Init(MyD3D& d3d)
{
	...
	setClocks();
	while (WinUtil::Get().BeginLoop(canUpdateRender) && state!=Exit)
	{
...
void gameManager::Run(MyD3D& d3d) 
{
	while (WinUtil::Get().BeginLoop(canUpdateRender) && state!=Exit)
	{
		deltaTime = WinUtil::Get().EndLoop(canUpdateRender);
...
void gameManager::render(MyD3D& d3d)
{
	...
	gpSpriteBatch->Begin(DirectX::SpriteSortMode_Deferred, 
		dxstate.NonPremultiplied(), 
		&d3d.GetWrapSampler());
	...
	if(state == State::Playing)
		font->DrawString(gpSpriteBatch, 
			"Which clock shows the same time as the digital one? 1, 2, or 3", 
			bottomTextPos);
	else
		font->DrawString(gpSpriteBatch, 
		(correctAnswer==true ? 
			"correct, try again? <y or n>" : 
			"wrong, try again?  <y or n>"), 
			bottomTextPos);
...
void gameManager::checkIfCorrect(const int code)
{
	...
	state = State::Limbo;
	if ((anClocks->at(clockNum)->getTime().getHours() == correctTime.getHours()) && 
		(anClocks->at(clockNum)->getTime().getMinutes() == correctTime.getMinutes()))
	{
		correctAnswer = true;
	}
	else
