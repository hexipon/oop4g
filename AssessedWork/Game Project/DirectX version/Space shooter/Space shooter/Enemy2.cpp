...
const void Enemy2::shoot()
{
	...
	Bullet mis3(_d3d, _spritebatch, deltaTime, "EnemyBullets", -1);
	bullets.push_back(mis3);
	bulletpos = DirectX::SimpleMath::Vector2(ship.GetPosition().x, ship.GetPosition().y + (ship.GetScreenSize().y / 2) + guns2Distance * (gameData::Get().WindowSize.x / gameData::Get().MaxWindowSize.x));
	bullets.back().Init(bulletpos, colour, Velocity.x*speed*(gameData::Get().WindowSize.x / gameData::Get().MaxWindowSize.x));
	missileTimer = GetClock() + missile_debounce;
	Bullet mis4(_d3d, _spritebatch, deltaTime, "EnemyBullets", -1);
	bullets.push_back(mis4);
	bulletpos = DirectX::SimpleMath::Vector2(ship.GetPosition().x, ship.GetPosition().y + (ship.GetScreenSize().y / 2) - guns2Distance * (gameData::Get().WindowSize.x / gameData::Get().MaxWindowSize.x));
	bullets.back().Init(bulletpos, colour, Velocity.x*speed*(gameData::Get().WindowSize.x / gameData::Get().MaxWindowSize.x));
	missileTimer = GetClock() + missile_debounce;
	return void();
...
const void Enemy2::render()
{
	ship.Draw(_spritebatch);
	return void();
}

const void Enemy2::update()
{
...
const void Enemy2::hasCollided(const int _colour)
{
	switch (colour)
	{
	case Ship::GREEN:
...
const bool Enemy2::checkCollision(const Sprite & sprite)
{
