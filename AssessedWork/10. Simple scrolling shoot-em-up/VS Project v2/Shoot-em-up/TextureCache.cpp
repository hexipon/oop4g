...
ID3D11ShaderResourceView* TextureCache::LoadTexture(ID3D11Device*pDevice, const std::string& fileName, const std::string& texName, bool appendPath, const vector<RECTF> *frames)
{
	string name = texName;;
	if (name.empty())
	{
...
const TextureCache::Data & TextureCache::Get(ID3D11ShaderResourceView * pTex) {
...
Vector2 TextureCache::GetDimensions(ID3D11ShaderResourceView* pTex)
{
	assert(pTex);
