...
void playState::update(const float & deltaTime, d3d& _d3d)
{
	...
	if (GameManager::Get().mouseAndKeyInput.IsPressed(VK_SPACE) && GetClock() > missileTimer)
	{
		Missile mis(_d3d);
		mis.Init(_d3d);
		mis.spr.GetAnim().Play(true);
		mis.spr.SetPosition(DirectX::SimpleMath::Vector2(Player.GetPosition().x + Player.GetScreenSize().x, Player.GetPosition().y + (Player.GetScreenSize().y / 2) - (mis.spr.GetScreenSize().y / 2)));
		mis.active = true;
	...
	if ( GameManager::Get().mouseAndKeyInput.IsPressed(VK_UP) ||
		 GameManager::Get().mouseAndKeyInput.IsPressed(VK_DOWN) ||
		GameManager::Get().mouseAndKeyInput.IsPressed(VK_W) ||
		GameManager::Get().mouseAndKeyInput.IsPressed(VK_S) ||
		(GameManager::Get().GamepadInput.IsConnected(0) &&
		(GameManager::Get().GamepadInput.GetState(0).leftStickX != 0 || GameManager::Get().GamepadInput.GetState(0).leftStickX != 0)))
	{
		//move the ship
		DirectX::SimpleMath::Vector2 pos{ Player.GetPosition() };
		if (GameManager::Get().mouseAndKeyInput.IsPressed(VK_W) || GameManager::Get().mouseAndKeyInput.IsPressed(VK_UP))
			pos.y -= SPEED * deltaTime;
		else if (GameManager::Get().mouseAndKeyInput.IsPressed(VK_S) || GameManager::Get().mouseAndKeyInput.IsPressed(VK_DOWN))
			pos.y += SPEED * deltaTime;

		if ( GameManager::Get().GamepadInput.IsConnected(0))
		{
			DBOUT("left stick x=" <<  GameManager::Get().GamepadInput.GetState(0).leftStickX << " y=" <<  GameManager::Get().GamepadInput.GetState(0).leftStickY);
			pos.y -=  GameManager::Get().GamepadInput.GetState(0).leftStickY * PAD_SPEED * deltaTime;
		}
...
{
	//load the ship
	Player.SetTex(*_d3d.GetCache().Get("ship").pTex);
	Player.SetScale(DirectX::SimpleMath::Vector2(0.2f, 0.2f));
	//setup play area
	int w, h;
